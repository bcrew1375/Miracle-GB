...
void RunEmulation()
{
	...
	if (logging == 1)
	{
		sprintf(logText, "PC = 0x%x  SP = 0x%x  AF = 0x%x  BC = 0x%x  DE = 0x%x  HL = 0x%x  Interrupts = %x  Opcode = 0x%x", regs.PC, regs.SP, regs.AF,
			regs.BC, regs.DE, regs.HL, interruptMasterEnable, opcode);
		WriteToLog(logText);
	}
